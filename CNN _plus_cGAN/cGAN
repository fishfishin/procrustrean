...
class CGAN():
    def __init__(self):
        ...
        self.optimizer = Adam(0.0007, 0.5)
    ...
    def build_generator(self):
        ...
        label1 = Reshape((16, 16, 1))(label1)

        l2 = Input(shape=(64,))
        #label2 = Embedding(10, 10 )(l2)
        label2 = Dense(n_nodes)(l2)
        label2 = Reshape((16, 16, 1))(label2)

        l3 = Input(shape=(64,))
        #label3 = Embedding(10, 10 )(l3)
        label3 = Dense(n_nodes)(l3)
        label3 = Reshape((16, 16, 1))(label3)

        merge = Concatenate()([gen, label1,label2,label3])
        '''
        l1 = Input(shape=(64,))
        l2 = Input(shape=(64,))
        l3 = Input(shape=(64,))
        ...
        label = Reshape((16, 16, 1))(label)
        merge = Concatenate()([gen, label])

        gen =Conv2DTranspose(128, kernel_size=3,strides=(2,2), padding="same")(merge)
        gen =LeakyReLU(alpha=0.2)(gen)
        #gen =BatchNormalization(momentum=0.8)(gen)

        gen = Conv2DTranspose(64, kernel_size=3, strides=(2,2),padding="same")(gen)
        gen = LeakyReLU(alpha=0.2)(gen)
        #gen = BatchNormalization(momentum=0.8)(gen)
        
        gen = Conv2DTranspose(32, kernel_size=3,strides=(2,2),padding="same")(gen)
        gen = LeakyReLU(alpha=0.2)(gen)
        #gen = BatchNormalization(momentum=0.8)(gen)

        gen = Conv2D(self.channels, kernel_size=3,padding='same')(gen)
        img = Activation("tanh")(gen)


        return Model([in_lat, l1,l2,l3], img)
    ...
    def build_discriminator(self):
        ...
        dis = Conv2D(128, kernel_size=3, strides=1, padding="same")(dis)
        dis = LeakyReLU(alpha=0.2)(dis)
        #dis = Dropout(0.25)(dis)
    ...
    def sample_images(self, epoch,gen_imgs):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("C:/Users/ZhenjuYin/Documents/Python Scripts/emotic/class/cgan/images/%d.png" % epoch)
        plt.close()

    def save_model(self):

        def save(model, model_name):
            model_path = "C:/Users/ZhenjuYin/Documents/Python Scripts/emotic/class/cgan/%s.json" % model_name
            weights_path = "C:/Users/ZhenjuYin/Documents/Python Scripts/emotic/class/cgan/%s_weights.h5" % model_name
...
if __name__ == '__main__':
    ...
    #cgan.discriminator.load_weights("C:/Users/ZhenjuYin/Documents/Python Scripts/emotic/class/cgan/discriminator_weights.h5")
    #cgan.generator.load_weights("C:/Users/ZhenjuYin/Documents/Python Scripts/emotic/class/cgan/generator_weights.h5")

    cgan.train(epochs=14000, batch_size=32, sample_interval=100)
