...
from keras.layers import Conv2D,Flatten,Dense,Layer,Reshape,Lambda
...
class repeat(Layer):
    ...
    def compute_output_shape(self, input_shape):
        ...
        return tensor_shape.TensorShape([input_shape[0], self.n,self.n, input_shape[1]])

    def call(self, inputs):
        label = K.expand_dims(inputs,axis=1)
        label = K.expand_dims(label, axis=1)
        label = K.tile(label, [1,self.n,self.n,1] )
        return label
...
def define_composite_model(g_model_1, d_model, g_model_2, image_shape):
	...
	gen1_out = g_model_1([input_gen,input_id])
	output_d = d_model([gen1_out,input_id])
	# identity element

	output_id = g_model_1([input_id,input_id])
	# forward cycle
	output_f = g_model_2([gen1_out,input_gen])
	# backward cycle
...
def generate_fake_samples(g_model, datasetA, datasetB, patch_shape):
...
def summarize_performance(step, g_model, trainX, trainY, name, n_samples=5):
	# select a sample of input images
	X_in, _ = generate_real_samples(trainX, n_samples, 0)
	Y_in, _ = generate_real_samples(trainY, n_samples, 0)
	# generate translated images
	X_out, _ = generate_fake_samples(g_model, X_in,Y_in,  0)
	X_in = (X_in + 1) / 2.0  # scale all pixels from [-1,1] to [0,1]
	X_out = (X_out + 1) / 2.0
	X_in = np.repeat(X_in,3,axis=3)
	X_out = np.repeat(X_out,3,axis=3)
...
def train(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA,trainA, trainB):
	...
	for i in range(n_steps):
		...
		X_fakeA, y_fakeA = generate_fake_samples(g_model_BtoA, X_realB,X_realA,n_patch)
		X_fakeB, y_fakeB = generate_fake_samples(g_model_AtoB, X_realA,X_realB, n_patch)
		...
		if (i+1) % (bat_per_epo * 1) == 0:
			...
			summarize_performance(i, g_model_AtoB, trainA, trainB, 'AtoB')
			# plot B->A translation
			summarize_performance(i, g_model_BtoA, trainB,trainA, 'BtoA')
...
def Encoder(image_shape):
    ...
    model.add(Conv2D(64, kernel_size=7, strides=1, input_shape=image_shape, padding="same", kernel_initializer=init, kernel_constraint=const))
...
image_shape = (64,64,1)
encoder_a =Encoder(image_shape)
encoder_b =Encoder(image_shape)

g_model_AtoB = define_generator(image_shape,encoder_b)

g_model_BtoA = define_generator(image_shape,encoder_a)

d_model_A = define_discriminator(image_shape,encoder_a)
# discriminator: B -> [real/fake]
d_model_B = define_discriminator(image_shape,encoder_b)
# composite: A -> B -> [real/fake, A]
c_model_AtoB = define_composite_model(g_model_AtoB, d_model_B, g_model_BtoA, image_shape)
# composite: B -> A -> [real/fake, B]
c_model_BtoA = define_composite_model(g_model_BtoA, d_model_A, g_model_AtoB, image_shape)
# train models
train(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA,fear,neutral)